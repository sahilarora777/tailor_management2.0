<x-layout bodyClass="g-sidenav-show bg-gray-200">
    ...
    <div class="main-content position-relative bg-gray-100 max-height-vh-100 h-100">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage='Manage products'></x-navbars.navs.auth>
        <!-- End Navbar -->
                                ...
                                <div class="col-md-8">
                                    ...
                                    <form action="{{ route('product.update', $product->id) }}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" id="rate" name="rate" class="form-control"  value="{{ $product->rate }}" required>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="categories_id" id="categories_id" class="form-control" required>
                                                @foreach($brands as $brand)
                                                    <option value="{{ $brand->id }}" {{ $brand->id == $product->categories_id ? 'selected' : '' }}>
                                                        {{ $brand->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="brands_id" id="brands_id" class="form-control" required>
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->id }}" {{ $category->id == $product->brands_id ? 'selected' : '' }}>
                                                        {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
